...
const Chat: React.FC = () => {
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      console.log(user.id)
      try {
        const response = await fetch(`https://cjxiaojia.com/api/en/message/${user_id}`, {
          headers: {
              ...
              <div key={message.id} className={`mb-4 ${message.receiver === 'receiver' ? 'text-right' : 'text-left'}`}>
                <div
                  className={`inline-block px-4 py-2 rounded-lg ${
                    message.receiver === 'receiver' ? 'bg-gray-300 text-black' : 'bg-indigo-500 text-white'
                  } max-w-[70%] w-auto`}
                >
                  <div
                    className="text-left"
                ...
                <div
                  className={`text-xs ${message.receiver === 'receiver' ? 'text-right' : 'text-left'} text-gray-500`}
                >
                  {message.time}
                </div>
              </div>
