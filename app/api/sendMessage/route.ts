...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!body.message || !body.receiver_id || !body.type) {
      return NextResponse.json({ message: "Missing required fields: message, receiverId, or type" }, { status: 400 });
    }
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ message: errorData || "Failed to create message" }, { status: response.status });
    }
